...
def confirmamt():
        ...
        cur.execute("""select fare,name from bus,operator where busid={} and bus_opid=opid""".format(bus_select.get()))
...
def showbuses():
        ...
        cur.execute(
            """select op.name,b.bus_type,r.seat_available,b.capacity,b.fare,b.bus_opid,st.stid as start_st,ed.stid as end_st from operator as op,bus as b,route as st,route as ed,runs as r 
            where r.runs_date='{}' and st.station_name="{}" and ed.station_name="{}" and st.stid< ed.stid and st.rid=ed.rid and b.bus_rid=st.rid and b.bus_opid=op.opid and r.runs_busid=b.busid""".format(
...
def proceedtobook():
    if bus_select.get() == 0:
        ...
        dated = datecorrect()
        cur.execute("""select b.fare,b.bus_opid,st.stid as start_st,ed.stid as end_st from bus as b,route as st,route as ed,runs as r where r.runs_date='{}' and st.stid="{}" and ed.stid="{}" and st.station_name< ed.station_name and st.rid=ed.rid and b.busid={}""".format(dated, fromf.get(), tof.get(), bus_select.get()))
