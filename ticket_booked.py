...
t2 = Label(root, text='Your Bus Booking', bg='SpringGreen2', fg='green4', font='Arial 22 bold')
...
cur.execute("SELECT * FROM bookinghistory where mobile={}".format(int(mob)))
f=cur.fetchall()
print(f)
...
Label(frame1,text='* Total amount Rs to be paid at the time of boarding the bus',fg='grey5',font='Arial 15').grid(row=11,columnspan=20,column=0)

print(gender)
print(passe)
#Label(frame1,text='Passenger'+str(f),  fg='White', font='Arial 12 bold').grid(row=4,column=6)


#Pname,mobile,age,seats,from_station,to_station,date_booked,ref_number,gender,boarding_date,fare,op_name
...
#mobf = Entry(root, textvariable=mobf)
